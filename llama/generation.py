...
class Llama:
   ...
   # 计算WPS
    ...
    # 数据处理和曲线绘制
    def create_plot(self,outputs, max_token, name):
         # 指定 data 文件夹的路径
        ...
        for x, y in zip(selected_x, selected_y_aver_efficiencies):
            ax3.annotate(f"{y:.2f}", (x, y), textcoords="offset points", xytext=(0,10), ha='center')
        ...
        # 设置总标题
        fig.suptitle(f"{name}(GPUS{num_gpus}) Generation Efficiency", fontsize=16, fontweight='bold')
